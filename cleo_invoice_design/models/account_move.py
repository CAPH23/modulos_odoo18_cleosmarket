...
class AccountMove(models.Model):
    ...
    def _cleo_delete_old_standard_invoice_attachments(self):
        """Delete old cached PDF attachments named like INV_2025_00023.pdf.

        Those attachments are the previous standard invoice PDF and are the ones
        that appear as a duplicate in the Send & Print wizard. The new Cleo PDF
        generated by the wizard is kept and renamed in the transient wizard.
        """
        Attachment = self.env['ir.attachment'].sudo()
        for move in self.sudo():
            if not move.name:
                continue
            expected = move.name.replace('/', '_').replace(' ', '_') + '.pdf'
            attachments = Attachment.search([
                ('res_model', '=', 'account.move'),
                ('res_id', '=', move.id),
                ('mimetype', '=', 'application/pdf'),
            ])
            old_attachments = attachments.filtered(
                lambda att: (att.name or '').lower() == expected.lower()
                and 'comprobante' not in (att.name or '').lower()
                and 'cleo' not in (att.name or '').lower()
            )
